...
export function Navigation() {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-white border-b px-4 md:px-6 h-16 flex items-center gap-3 md:gap-4 shadow-sm">
      <div className="flex items-center gap-2 shrink-0">
        ...
        <Link href="/" className="flex items-center gap-1.5 group">
          ...
          <div className="hidden lg:flex flex-col -space-y-1 ml-1">
            <span className="font-headline text-[13px] font-black tracking-tighter text-black uppercase leading-none whitespace-nowrap">
              Steak West
            </span>
            <span className="text-[8px] font-black text-primary uppercase tracking-[0.2em] whitespace-nowrap">Super ya Nyama</span>
          </div>
        </Link>
      </div>
      ...
      <div className="hidden xl:flex items-center gap-1.5 px-4 py-2 bg-gray-50 border border-black/5 rounded-none hover:bg-gray-100 cursor-pointer shrink-0 transition-all">
        ...
        <span className="text-[10px] font-black uppercase tracking-widest whitespace-nowrap">Utalii St • ASAP</span>
      ...
      <div className="flex-grow relative max-w-xl group mx-2 hidden sm:block">
        ...
        <Input 
          className="w-full bg-gray-50 border-none h-10 pl-11 text-[13px] font-bold placeholder:font-medium focus-visible:ring-1 focus-visible:ring-primary/10 transition-all rounded-none"
      ...
      <div className="flex items-center gap-2 md:gap-4 shrink-0 ml-auto">
        ...
        <Link href={user ? "/profile" : "/login"}>
          <Button 
            ...
            size="icon" 
            className={cn(
              "h-10 w-10 rounded-none transition-all border group",
              !user ? "bg-white border-black hover:bg-black hover:text-white" : "bg-gray-50 text-black border-black/10 hover:border-black"
            )}
          >
            {user ? <User className="w-5 h-5" /> : <LogIn className="w-5 h-5 transition-transform group-hover:translate-x-1" />}
          </Button>
        </Link>

        <Link href="/checkout">
          <Button variant="ghost" size="icon" className="relative bg-black text-white hover:bg-primary h-10 w-10 shadow-xl shadow-black/10 transition-all active:scale-90 rounded-none border-none">
            <ShoppingCart className="w-5 h-5" />
            <span className="absolute -top-1 -right-1 bg-primary text-white text-[9px] w-5 h-5 flex items-center justify-center font-black border-2 border-white">
              {itemCount}
            </span>
          </Button>
        </Link>
