...
import { User as UserIcon, LogOut, Shield, Calendar, Clock, LogIn, Settings } from "lucide-react";
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
        <Card className="max-w-md w-full p-8 text-center rounded-[2.5rem] border-2 shadow-xl">
          ...
          <p className="text-muted-foreground mb-8 text-[14px]">Please sign in to view your profile and manage orders.</p>
          <Button className="w-full h-14 rounded-2xl gap-2 text-[14px] shadow-lg shadow-primary/20" onClick={() => router.push("/login")}>
        ...
        <div className="lg:col-span-1">
          <Card className="border-none shadow-2xl overflow-hidden rounded-[2.5rem] bg-card">
            ...
            <CardContent className="pt-16 pb-8 text-center px-8">
              ...
              <p className="text-[14px] text-muted-foreground mb-6">{user.email}</p>
              
              <Badge variant="secondary" className="mb-8 bg-primary/10 text-primary border-none px-6 py-2 flex items-center gap-2 w-fit mx-auto capitalize rounded-full font-bold text-[14px]">
                <Shield className="w-4 h-4" /> {role}
              </Badge>

              <div className="space-y-2">
                <Button variant="outline" className="w-full justify-start gap-3 rounded-xl h-12 border-primary/10 hover:bg-primary/5 text-[14px] font-bold">
                  <Settings className="w-4 h-4 text-primary" /> Account Settings
                </Button>
                <Button 
                  variant="ghost" 
                  className="w-full justify-start gap-3 rounded-xl h-12 text-destructive hover:bg-destructive/5 hover:text-destructive text-[14px] font-bold"
