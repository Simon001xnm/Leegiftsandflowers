        ...
        {CATEGORIES.map((category) => {
          ...
          return (
            <section key={category.id} id={category.id} className="scroll-mt-40">
              ...
              {/* HIGH DENSITY FLEX GRID: 50% width on mobile (2 per row), 16.6% on tablet (6), 12.5% on laptop (8) */}
  ...
  return (
    <div className="w-full h-full flex flex-col group cursor-pointer overflow-hidden transition-all duration-300 bg-white hover:bg-gray-50">
      <div className="aspect-square relative bg-gray-50 overflow-hidden shrink-0">
        ...
        />
        {product.isHalal && (
          <div className="absolute top-1 left-1 z-10 w-6 h-6 md:w-8 md:h-8 rounded-full overflow-hidden border border-white shadow-lg bg-white">
            <Image src="/images (45).jpg" alt="Halal Certified" fill className="object-contain p-0.5" />
          </div>
        )}
        <button 
          onClick={onAdd} 
          className="absolute bottom-2 right-2 w-8 h-8 md:w-9 md:h-9 bg-white/95 backdrop-blur-sm rounded-full flex items-center justify-center text-primary shadow-lg transition-all z-20 active:scale-90 border border-gray-100"
        >
          <Plus className="w-4 h-4 md:w-5 md:h-5 stroke-[3px]" />
        </button>
      </div>
      <div className="p-3 md:p-4 flex-grow flex flex-col justify-between space-y-1 min-w-0">
        <div className="space-y-0.5 min-w-0">
          <p className="text-[13px] md:text-[14px] font-black uppercase tracking-tighter truncate leading-tight group-hover:text-primary transition-colors">{product.name}</p>
        </div>
