...
function DiscoveryContent() {
  ...
  return (
    <main className="flex-grow bg-white pb-20">
      {/* Category Strip - Exact spacing from reference */}
      <div className="sticky top-16 z-40 bg-white border-b overflow-x-auto no-scrollbar py-4 px-4 md:px-8">
        <div className="flex items-center gap-8 min-w-max container mx-auto">
          {CATEGORIES.map((cat) => (
            ...
            >
              <div className="w-10 h-10 rounded-full flex items-center justify-center transition-all bg-gray-50 group-hover:bg-gray-100">
      ...
      <div className="container mx-auto px-4 md:px-8 space-y-10 py-6">
        {/* Promo Banners - Exact ratio from reference */}
        <section className="flex gap-4 overflow-x-auto no-scrollbar mask-fade-right">
          {[1, 2, 3].map((i) => (
            <div key={i} className={cn(
              "min-w-[300px] md:min-w-[400px] h-40 rounded-2xl p-6 flex flex-col justify-between relative overflow-hidden",
              i === 1 ? "bg-orange-100" : i === 2 ? "bg-blue-50" : "bg-red-50"
            )}>
              <div className="relative z-10 space-y-1">
                <p className="text-[9px] font-black uppercase tracking-[0.15em] text-primary">Member Special</p>
                <h3 className="text-lg font-black max-w-[180px] leading-tight uppercase">Steak West Prime free for 4 weeks</h3>
                <p className="text-[10px] font-bold text-gray-500">Limited time only</p>
              </div>
              <Button size="sm" className="w-fit rounded-full px-4 h-7 text-[10px] font-black bg-black text-white uppercase tracking-wider">Order Now</Button>
              <div className="absolute right-[-10px] bottom-[-10px] w-32 h-32 bg-white/20 rounded-full" />
            </div>
          ))}
        </section>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
               <Link href="/restaurants">
                 <span className="text-[12px] font-bold text-black mr-2">See all</span>
               </Link>
               <Button variant="ghost" size="icon" className="rounded-full h-7 w-7 bg-gray-50"><ChevronRight className="w-3 h-3 rotate-180" /></Button>
               <Button variant="ghost" size="icon" className="rounded-full h-7 w-7 bg-gray-50"><ChevronRight className="w-3 h-3" /></Button>
            </div>
          </div>
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 mask-fade-right">
            {MOCK_RESTAURANTS.map((r) => (
              <StoreCard key={r.id} restaurant={r} className="min-w-[280px] md:min-w-[320px]" showBadge />
            ))}
          </div>
        </section>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/restaurants"><span className="text-[12px] font-bold text-black">See all</span></Link>
          </div>
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 mask-fade-right">
            {MOCK_RESTAURANTS.map((r) => (
              <StoreCard key={r.id} restaurant={r} className="min-w-[280px] md:min-w-[320px]" isOffer />
            ))}
          </div>
        </section>

        {/* Circular Brand Section - Exact ratio from reference */}
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-black tracking-tighter uppercase">Stores near you</h2>
            <Link href="/restaurants"><span className="text-[12px] font-bold text-black">See all</span></Link>
          </div>
          <div className="flex gap-6 overflow-x-auto no-scrollbar py-4 justify-between">
            {['Steak West', 'Choma Grill', 'Butchery', 'Kachumbari', 'Drinks', 'Grocery'].map((brand, i) => (
              <div key={brand} className="flex flex-col items-center gap-2 group cursor-pointer shrink-0">
                ...
                <span className="text-[12px] font-bold text-center tracking-tight">{brand}</span>
    ...
    <Link href={`/restaurants/${restaurant.id}`} className={cn("group flex flex-col gap-2", className)}>
      <div className="relative aspect-[16/9] rounded-xl overflow-hidden bg-gray-100">
        ...
        {isOffer && (
          <div className="absolute top-2.5 left-2.5">
             <Badge className="bg-primary text-white border-none text-[9px] font-black tracking-widest rounded-md py-1 px-2.5 uppercase">
               BUY 1, GET 1 FREE
             </Badge>
          </div>
        )}
        ...
        {showBadge && !isOffer && (
          <div className="absolute top-2.5 left-2.5">
             <Badge className="bg-emerald-500 text-white border-none text-[9px] font-black tracking-widest rounded-md py-1 px-2.5 uppercase">
               KES 121 OFF
             </Badge>
          </div>
        )}
      ...
      <div className="space-y-0.5 px-0.5">
        <div className="flex justify-between items-start">
          <h3 className="text-[15px] font-black truncate leading-tight tracking-tight uppercase">{restaurant.name}</h3>
        </div>
        <div className="flex items-center gap-1.5 text-[12px] font-bold text-gray-500">
           <div className="flex items-center gap-1">
             ...
             <span>KES {restaurant.deliveryFee} FEE</span>
           </div>
        ...
        {restaurant.isFeatured && (
          <div className="mt-1.5">
            <Badge className="bg-gray-100 text-gray-600 hover:bg-gray-100 border-none text-[9px] font-black uppercase rounded-sm h-5 px-1.5">
              GREAT VALUE
            </Badge>
          </div>
        )}
